...
public class AQIValue5dProcessOperator extends KeyedProcessOperator<String, AQIValue24h, AQIValue5d> {
    ...
    public void processElement(StreamRecord<AQIValue24h> value) throws Exception {
        ...
        if (value.getValue().isWatermark()) { // emit results on watermark arrival or window has no slices pass watermark through
            if (window == null || window.getCheckpoint() < 0) { // in case no records are processed beforehand watermark has all values
